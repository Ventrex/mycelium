...
- **Separate Discord webhooks for movies and shows**: movie notifications can go to `DISCORD_WEBHOOK_URL_MOVIES` and series to `DISCORD_WEBHOOK_URL_SHOWS`, each falling back to the shared `DISCORD_WEBHOOK_URL` when left empty. Configurable under Settings > Notifications.
