...
- **Auto-Approve fills all genres at once into one combined daily budget**: the daily fill now visits every "Auto-fill trending" genre across both the Movies and Shows tabs round-robin, into a single shared budget of `AUTO_APPROVE_DAILY_LIMIT_MOVIE` titles (movies + series mixed), with `AUTO_APPROVE_DAILY_LIMIT_TV` as an optional cap on the series share. Previously the fill drained genre by genre, so with a budget of 100 only the first couple of genres ever got filled and a second run found nothing; now all genres are represented every run and repeated runs keep pulling fresh titles.
