...
- **Zilean is now built into Mycelium natively**: the external `iPromknight/zilean` container + Postgres dependency is gone. Mycelium now downloads and indexes the community-shared DMM hashlists (github.com/debridmediamanager/hashlists) itself, in its own SQLite file (`ZILEAN_DB_PATH`, default `/data/zilean.db`), with a background sync job (`ZILEAN_SYNC_INTERVAL_HOURS`, default 12h). No separate container to run, no profile flag needed - just set `ZILEAN_ENABLED=true`. Also fixes a bug where `ZILEAN_URL`/`ZILEAN_ENABLED` changed via the Settings UI were silently ignored because several code paths read the frozen startup value instead of the live setting, which made Zilean look permanently "inactive" for anyone who configured it after first boot.
...
- **Rate-limited series episodes could sit for hours instead of retrying**: a rate-limited movie was already handed to the retry queue and retried within minutes, but a rate-limited episode just sat in `wanted_episodes` until the next 6-hourly series monitor run - so series fell behind films whenever the shared TorBox budget (60 createtorrent calls/hour) ran out. Episode retries now also go through the retry queue (`retry_queue.schedule_episode`), so they're picked up again within `RETRY_QUEUE_INTERVAL_MINUTES` like a movie. The retry queue also now processes manually-requested items before anything queued by auto-approve/trending (oldest-first within each group), so a user's own click never gets stuck behind a backlog of background auto-fill.
- **Episode playback returned a 400 / "playback error" while movies worked**: the catbox stream proxy cached each TorBox CDN link for 23 hours, but the underlying torrent can disappear sooner (idle-release or TorBox eviction), after which the cached link 400s - and the proxy kept serving it. Episodes (idle season packs) hit this far more than movies. The link cache is now 1 hour, so a stale link self-heals on the next play instead of erroring for hours. In addition, TorBox `requestdl` calls are throttled and retried on 429, so the preload/probe bursts no longer exhaust the rate limit and leave us unable to mint a fresh link. The per-request `/stream` log line is now DEBUG for cache hits (it was flooding the log on every player retry).
