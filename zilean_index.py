"""Native DMM hash index -- replaces the external iPromKnight/zilean service.
...
Postgres, no network dependency at query time.
"""
import json
import logging
import os
...
def search(title: str, season: int | None = None, episode: int | None = None, limit: int = 200) -> list[dict]:
    ...
    try:
        rows = (
            _fts_query(conn, tokens, fetch_limit) if _fts_available
